...
export default async function SabitSayfa() {
  const response = await getStaticPage(PageTypeEnum.PageUsagePolicy);
  ...
  if ("data" in response) {
    ...
    metadata.title = PageTypeEnumDisplayNames[PageTypeEnum.PageUsagePolicy];
  ...
  return (
    <>
      <head>
        <title>{PageTypeEnumDisplayNames[PageTypeEnum.PageUsagePolicy]}</title>
      </head>
