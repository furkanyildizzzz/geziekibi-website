...
export default async function SabitSayfa() {
  const response = await getStaticPage(PageTypeEnum.PageAboutUs);

  let pageData = {
    pageType: PageTypeEnum.PageAboutUs,
  } as StaticPageSuccessResponse;
  if ("data" in response) {
    pageData = response.data;
    metadata.title = PageTypeEnumDisplayNames[PageTypeEnum.PageAboutUs];
  ...
  return (
    <>
      <head>
        <title>{PageTypeEnumDisplayNames[PageTypeEnum.PageAboutUs]}</title>
      </head>
