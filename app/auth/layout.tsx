...
import authImage from "../../public/auth-image-3d-cartoon.jpg";
...
const AuthLayout = ({ children }: { children: React.ReactNode }) => {
  ...
  const layoutComponent = (
    <div className="flex items-center justify-center  flex-row-reverse h-screen  w-full   ">
      <div className="w-full p-2  flex-1 flex relative flex-col   items-center mt-2 md:mt-0  ">
        ...
        <p className="max-w-sm test-sm text-center text-white  md:text-black md:dark:text-white ">
      ...
      <div
        className={`fixed left-0 top-0 brightness-[8%] md:brightness-90 md:relative bg-cover  md:flex justify-center items-center  bg-center w-full md:flex-1 overflow-hidden  brightness-70 border-r-violet-800 border-r  h-screen   -z-10  ${
          loading && "bg-gradient-to-r from-violet-400 to-violet-800 "
        } `}
      >
        <Image
          src={authImage}
  ...
  return <CaptchaClientProvider>{layoutComponent}</CaptchaClientProvider>;
};
