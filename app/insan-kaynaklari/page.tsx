...
export default async function SabitSayfa() {
  const response = await getStaticPage(PageTypeEnum.PageHumanResources);
  ...
  if ("data" in response) {
    ...
    metadata.title = PageTypeEnumDisplayNames[PageTypeEnum.PageHumanResources];
  ...
  return (
    <>
      <head>
        <title>
          {PageTypeEnumDisplayNames[PageTypeEnum.PageHumanResources]}
        </title>
      </head>
