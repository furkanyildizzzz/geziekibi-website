...
import { getTours } from "@/app/api/tour/getTours";
...
interface TourListParams {
  tourList: FeaturedTourListSuccessResponse[];
  category: CategoryListSuccessResponse | null;
}

const TourList = ({ tourList, category }: TourListParams) => {
  ...
  const calculateMinAndMaxFilterPrice = (
    ...
    const { minPrice, maxPrice } = toursData.reduce(
      (acc, tour) => {
        acc.minPrice = Math.min(acc.minPrice, tour.pricePerPerson || 100);
        acc.maxPrice = Math.max(acc.maxPrice, tour.pricePerPerson || 100);
        return acc;
      },
    ...
    console.log("sdcfsd", { minPrice, maxPrice });
  ...
  useEffect(() => {
    if (startDate || destination || category) {
      handleSearch({
        startDate: startDate,
        destination: {
