...
function RTE_Plugin_Html2PDF() {
  ...
  obj.DoHtml2PDF = function () {
        ...
        .then(function (img) {
          ...
          prom
            .then(function (str) {
              ...
              div2.innerHTML =
                Math.ceil(str.length / 1024) +
                "KB Exported<br/><a href='" +
                url +
              ...
              img.style.cssText =
                "max-width:300px;max-height:200px;margin:5px;padding:10px;border:solid 1px #eee;box-shadow:2px 2px 6px #eee";
    ...
    iframe = __Append(
      div1,
      "iframe",
      "align-self:center;flex:99;width:100%;height:0px;border:0px;",
      "rte-editable"
    );
    iframe.contentDocument.open("text/html");
    iframe.contentDocument.write(
      "<html><head><link id='url-css-preview' rel='stylesheet' href='" +
        editor.htmlEncode(config.previewCssUrl) +
        "'/>" +
        "<script src='" +
        editor.htmlEncode(scripturl) +
        "'></script></head><body style='padding:10px;margin:0px'>" +
        editor.getHTMLCode() +
        "</body>" +
...
function RTE_Plugin_ImageEditor() {
  ...
  obj.DoImageEditor = function () {
    ...
    window.rte_image_editor_callback = function (win) {
      ...
      var imgeditor = new win.tui.ImageEditor(
        win.document.querySelector("#tui-image-editor"),
        options
      );
    ...
    iframe = __Append(
      dialoginner,
      "iframe",
      "align-self:center;flex:99;width:100%;height:100%px;border:0px;",
      "rte-editable"
    );
    iframe.contentDocument.open("text/html");
    iframe.contentDocument.write(
      "<html><head><base href='" +
        editor.htmlEncode(location.href) +
        "'/>" +
        "<link id='url-css-preview' rel='stylesheet' href='" +
        editor.htmlEncode(scriptbase + "tui-color-picker.min.css") +
        "'/>" +
        "<link id='url-css-preview' rel='stylesheet' href='" +
        editor.htmlEncode(scriptbase + "tui-image-editor.min.css") +
        "'/>" +
        "<script src='" +
        editor.htmlEncode(scriptbase + "fabric.min.js") +
        "'></script>" +
        "<script src='" +
        editor.htmlEncode(scriptbase + "tui-color-picker.min.js") +
        "'></script>" +
        "<script src='" +
        editor.htmlEncode(scriptbase + "tui-code-snippet.min.js") +
        "'></script>" +
        "<script src='" +
        editor.htmlEncode(scriptbase + "tui-image-editor.min.js") +
        "'></script>" +
...
function RTE_Plugin_InsertCode() {
  var dp = {
    sh: { Toolbar: {}, Utils: {}, RegexLib: {}, Brushes: {}, Version: "1.5.1" },
  };
  dp.InsertCode = dp.sh;
  ...
  obj.DoShowDialog = function () {
    var dialoginner = editor.createDialog(
      editor.getLangText("insertcode"),
      "rte-dialog-insertcode"
    );
    ...
    var textarea = __Append(
      div1,
      "textarea",
      "width:100%;min-width:300px;height:200px"
    );

    var divfooter = __Append(
      dialoginner,
      "rte-dialog-footer",
      null,
  ...
  obj.InitEditor = function (argeditor) {
    ...
    editor.toolbarFactoryMap["insertemoji"] = function (cmd) {
      return editor.createToolbarItemDropDownPanel(cmd, function (panel) {
        ...
        var searchbar = __Append(
          panel,
          "label",
          "margin:5px;position:relative;"
        );
        searchbar.setAttribute("id", "emojis_searchbar");
        var searchbox = __Append(
          searchbar,
          "input",
          "width:100%;padding:5px 20px;border:solid 1px #ccc;border-radius:5px;"
        );
        searchbox.setAttribute(
          "placeholder",
          editor.getLangText("searchemojis")
        );
        ...
        function show_result() {
          ...
          var resultline = __Append(
            resultpanel,
            "div",
            "width:100%;padding:3px;margin-top:5px;color:darkblue;text-align:center;"
          );
        ...
        var resultpanel = __Append(
          panel,
          "div",
          "display:none;flex-direction:row;flex-wrap:wrap;overflow-y:scroll;padding-bottom:55px"
        );

        var grouppanel = __Append(
          panel,
          "div",
          "overflow-y:scroll;padding-bottom:55px;flex:999"
        );
        ...
        for (var gi = 0; gi < emojidata.length; gi++) {
          ...
          var gdiv = __Append(
            grouppanel,
            "div",
            "padding:3px;margin-top:5px;color:darkblue;"
          );
          groupdivs.push(gdiv);
          gdiv.innerText =
        ...
        function CreateTabBtn(group) {
          var btn = __Append(
            ...
            "width:32px;text-align:center;margin:4px"
          );
          tabuibtns.push(btn);
          btn.setAttribute("title", group.name);
          btn.innerHTML = group.items[0].emoji;
          btn.onclick = function () {
            grouppanel.scrollTop =
              groupdivs[group.index].getBoundingClientRect().top -
              grouppanel.getBoundingClientRect().top +
              grouppanel.scrollTop;
            grouppanel.onscroll();
          };
          btn.group = group;
...
function RTE_Plugin_InsertGallery() {
  ...
  obj.DoInsertGallery = function () {
    var dialoginner = editor.createDialog(
      editor.getLangText("insertgallerytitle"),
      "rte-dialog-insertgallery"
    );

    var scrollpanel = __Append(
      dialoginner,
      "rte-insertgallery-scrollpanel",
      "overflow-y:auto;padding-bottom:32px;"
    );
    var container = __Append(
      scrollpanel,
      "rte-insertgallery-container",
    ...
    function CreateDiv(item) {
      ...
      var divitem = __Append(
        container,
        "rte-insertgallery-image-item",
        "cursor:pointer;width:128px;height:128px;margin:12px;box-shadow:0 0 8px #ccc;display:flex;align-items:center;justify-content:center;"
      );
...
function RTE_Plugin_InsertTemplate() {
  ...
  obj.DoInsertTemplate = function () {
    var dialoginner = editor.createDialog(
      editor.getLangText("inserttemplatetitle"),
      "rte-dialog-inserttemplate"
    );

    var scrollpanel = __Append(
      dialoginner,
      "rte-inserttemplate-scrollpanel",
      "overflow-y:auto;padding-bottom:32px;"
    );
    var container = __Append(
      scrollpanel,
      "rte-inserttemplate-container",
      "display:flex;flex-wrap:wrap;"
    );
    ...
    function CreateDiv(item) {
      ...
      var divitem = __Append(
        container,
        "rte-inserttemplate-image-item",
        "cursor:pointer;width:256px;height:256px;margin:12px;box-shadow:0 0 8px #ccc;display:flex;align-items:center;justify-content:center;"
      );
      var div = __Append(
        divitem,
        "div",
